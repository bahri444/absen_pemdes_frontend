  ...
  <div>
    <NuxtLayout name="sidebar">
      <div class="container flex-1">
        <p class="font-semibold text-center mt-2 mb-5">Pegawai</p>
        <NuxtLink
          to="/superadmin/pegawai/addpegawai"
          ...
          <table class="w-full text-sm text-left text-slate-500">
            <thead
              ...
              <tr>
                ...
                <th scope="col" class="px-6 py-4">Dusun</th>
              ...
              <tr
                ...
                <td class="px-3 py-2 mx-auto">
                  <div class="grid lg:grid-cols-2 md:grid-cols-1 w-28 mx-auto">
                    <NuxtLink
                      :to="'/superadmin/pegawai/update/' + row.pegawai_uuid"
                    ...
                    <button
                      type="button"
                      @click="
                        ($event) => deleteEmploye($event, row.pegawai_uuid)
                      "
...
<script>
  ...
  export default {
    ...
    methods: {
      GetAllEmployee() {
        axios.get(urlapi + "/api/pegawai").then((res) => {
          this.allemploye = res.data;
        });
      },
      deleteEmploye(event, employeByUuid) {
        event.target.innerText = "Delete pegawai";
        axios
          .delete(urlapi + `/api/deletepegawai/${employeByUuid}`)
          .then((res) => {
