<template>
  <NuxtLayout name="sidebar">
    <div class="container flex-1">
      <p class="font-semibold text-center mt-2 mb-5">Absen hari ini</p>
      <NuxtLink
        to="/superadmin/absen/addabsen"
        ...
        <table class="w-full text-sm text-left text-slate-500">
          <thead
            ...
            <tr>
              ...
              <th scope="col" class="px-6 py-4">Jarak koordinat</th>
              <th scope="col" class="px-6 py-4">Jam masuk</th>
              <th scope="col" class="px-6 py-4">Jam pulang</th>
              <th scope="col" class="px-6 py-4">Aksi</th>
            </tr>
          </thead>
          <tbody id="example">
            <tr
              v-for="(row, index) in absens"
              :key="row.absen_uuid"
              class="bg-white border-b hover:bg-slate-100 text-center">
              <td class="px-6 py-4">{{ index + 1 }}</td>
              <td
                class="px-6 py-4"
                v-for="newrows in row.join_to_user_one_to_many">
                {{ newrows.nama_lengkap }}
              </td>
              <td class="px-6 py-4">{{ row.tgl_absen }}</td>
              <td class="px-6 py-4">{{ row.jarak_koordinat }}</td>
              ...
              <td class="px-3 py-2 mx-auto">
                <div class="grid lg:grid-cols-2 md:grid-cols-1 w-28 mx-auto">
                  <NuxtLink
                    :to="'/superadmin/pegawai/update/' + row.user_uuid"
                  ...
                  <button
                    type="button"
                    @click="($event) => deleteEmploye($event, row.user_uuid)"
...
<script>
  ...
  export default {
    ...
    methods: {
      GetAllAbsen() {
        axios.get(urlapi + "/api/allabsen").then((res) => {
          this.absens = res.data;
        });
      },
      deleteEmploye(event, AbsenByUuid) {
        event.target.innerText = "Delete pegawai";
        axios.delete(urlapi + `/api/deleteabsen/${AbsenByUuid}`).then((res) => {
          event.target.innerText = "deleted emplotye";
          this.GetAllAbsen();
        });
      },
    },
