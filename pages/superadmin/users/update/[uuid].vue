          ...
          <div class="px-4 md:px-0 lg:w-4/12 shadow-xl">
            <div class="md:mx-6 md:p-12">
              ...
              <div class="text-center">
                <h4 class="mb-12 mt-1 pb-1 text-xl font-semibold text-teal-600">
                  Update akun
                </h4>
              </div>

              <form @submit.prevent="PostUpfateAccount">
                <!--NIP-->
                <div class="relative mb-4" data-te-input-wrapper-init>
                  ...
                  <input
                    type="text"
                    v-model="updateAccount.nama_lengkap"
                ...
                <div class="relative mb-4" data-te-input-wrapper-init>
                  ...
                  <select
                    ...
                    name="pilih jabatan"
                    class="bg-white appearance-none border-2 border-teal-400 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-teal-50 focus:border-teal-500">
                    <option :value="updateAccount.role" selected>
                      {{ updateAccount.role }}
                    </option>
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      this.GetAccountByUuid(this.getByUuid);
    },
    methods: {
      GetAccountByUuid(getByUuid) {
        axios.get(urlapi + `/api/userbyuuid/${getByUuid}`).then((res) => {
          this.updateAccount = res.data;
        });
      },
      PostUpfateAccount() {
        axios
          .put(urlapi + `/api/updateuser/${this.getByUuid}`, this.updateAccount)
          .then((res) => {
